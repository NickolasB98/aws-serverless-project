...
SELECT
  -- Completeness checks
  SUM(CASE WHEN temp_c_max IS NULL THEN 1 ELSE 0 END) AS missing_temp_c_max,
  SUM(CASE WHEN temp_c_min IS NULL THEN 1 ELSE 0 END) AS missing_temp_c_min,
  SUM(CASE WHEN daylight_duration_hours IS NULL THEN 1 ELSE 0 END) AS missing_daylight_duration_hours,
  SUM(CASE WHEN sunshine_duration_hours IS NULL THEN 1 ELSE 0 END) AS missing_sunshine_duration_hours,
  ...
  SUM(CASE WHEN temp_c_min < -30 OR temp_c_max > 50 THEN 1 ELSE 0 END) AS extreme_temp_c,
...
FROM "de_proj_database"."open_meteo_forecast_weather_data_parquet_tbl"
"""
